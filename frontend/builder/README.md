...
### Run in Platform / hosted mode

Use [index.html](index.html) as the Builder entrypoint in platform deployments.
...
- `block` (block_component_type can be `rdm-trial`, `rdm-practice`, `rdm-adaptive`, `rdm-dot-groups`, `html-keyboard-response`, `html-button-response`, or `image-keyboard-response`)
  ...
  - Direction transitions within an RDM Block can be controlled with `direction_transition_mode` (`random_each_trial`, `no_repeat_each_trial`, `every_n_trials`, `exact_count`) plus `every_n`/`count` controls. `no_repeat_each_trial` samples randomly from the configured directions but never repeats the immediately preceding trial's direction.
